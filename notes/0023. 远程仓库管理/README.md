...
- [5. 🤔 Git 将远程仓库的相关配置数据写入到了什么地方？](#5--git-将远程仓库的相关配置数据写入到了什么地方)
- [6. 🤔 本地仓库和远程仓库的映射关系是？](#6--本地仓库和远程仓库的映射关系是)
...
remote 信息的写入通常只会在 Git 仓库初始化的时候做一次，后续直接通过命令 `git push`、`git pull` 来和远程仓库通信即可。

个人最常用的命令主要是查看远程仓库的信息：

- `git remote -v`，查看远程仓库的地址
- `git remote show xxx`，查看特定远程仓库的信息

在开始编辑一个 Git 仓库之前，我们通常学先查看 Git 远程仓库的状态（`git status` 或者 `git remote show origin`），判断本地的版本是否落后于远程仓库中的版本，如果落后，通常需要先 `git pull` 拉取最新的版本信息，再进行后续的编辑。
...
同理，如果你要移除某个特定的 remote，直接将配置文件中对应的 remote 数据删除即可。
...
```text
  +----------------+      +----------------+      +----------------+
  |  本地仓库 A    |      |  本地仓库 B    |      |  本地仓库 C    |
  | (开发者 Alice) |      | (开发者 Bob)   |      | (CI/CD 服务器) |
  +-------+--------+      +-------+--------+      +-------+--------+
          |                       |                       |
          | (git push/pull)       | (git push/pull)       | (git fetch/clone)
          v                       v                       v
  +---------------------------------------------------------------+
  |                    远程仓库 (Remote Central)                  |
  |                     (例如: GitHub Repo)                       |
  |                                                               |
  |  [分支 main] <--- 接收来自 A, B 的推送                         |
  |  [分支 dev ] <--- 接收来自 A, B 的推送                         |
  |  [保护规则] ---> 限制直接推送，需 Pull Request                 |
  +---------------------------------------------------------------+
          ^                       ^                       ^
          |                       |                       |
  +-------+--------+      +-------+--------+      +-------+--------+
  |  本地仓库 D    |      |  本地仓库 E    |      |  本地仓库 F    |
  | (开发者 Carol) |      | (新入职员工)   |      | (测试环境)     |
  +----------------+      +----------------+      +----------------+

  (所有本地仓库通过 git clone 初始化，或通过 git remote add 关联到同一个 URL)
```
...
```bash
git branch -vv
# * main e8fbbc7 [origin/main] 📝 Update notes - 2026-03-13 17:55:32
```
